...
#### 插件化架构与数据源扩展（钉钉数据源插件） 🔥 开发中（第一优先级）

**功能定位**：
为小遥搜索添加钉钉文档数据源识别能力，通过解析钉钉导出工具自动生成的 `.xyddjson` 元数据文件，实现零配置、自动识别的钉钉文档智能搜索体验。

**核心价值**：
- 零配置使用：用户导出文档后系统自动识别，无需任何配置
- 性能优越：独立元数据文件解析速度快于飞书方案（<1ms vs ~1ms）
- 可扩展性强：`.xyddjson` 格式可复用到其他平台（企业微信、飞书等）
- 提升搜索体验：在搜索结果中清晰标识钉钉文档来源，支持快速跳转原文

**技术实现**：
- **插件化框架**：基于Python ABC实现数据源插件基类
- **元数据解析**：解析钉钉导出工具生成的独立 `.xyddjson` JSON文件
- **零配置设计**：无需配置文件，自动识别元数据文件
- **文件扫描集成**：集成到 `FileScanner` 中自动检测

**技术优势**（相比飞书）：
- 独立JSON元数据文件，结构化、易解析
- 标准JSON格式，容错性强，解析失败不影响原文件
- 无需读取文件内容，性能更好
- 可扩展到其他平台，通用性强
