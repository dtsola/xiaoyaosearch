...
### 9.3 插件化架构与数据源扩展（语雀+飞书+钉钉） 🚧 规划中

**功能定位**：建立插件化架构，实现数据源的热插拔和独立开发，优先支持语雀知识库数据源、飞书文档数据源和钉钉文档数据源。

**技术要点**：
- **技术选型**：Python ABC + importlib + Pydantic + httpx + PyYAML + JSON
...
- **钉钉特色**：独立 `.xyddjson` 元数据文件解析，零配置自动识别
...
📖 **详细技术方案**：
- [插件化架构与语雀数据源技术方案](特性开发/plugins+yuque/plugins+yuque-03-技术方案.md)
- [飞书文档数据源技术方案](特性开发/plugins+feishu/plugins+feishu-03-技术方案.md)
- [钉钉文档数据源技术方案](特性开发/plugins+dingding/plugins+dingding-03-技术方案.md)
...
**功能定位**：建立多术语库集合系统，支持用户自定义领域术语库（医疗、法律、IT等），在搜索时选择性使用，实现精准查询扩展。核心解决当前BGE-M3向量模型对英文缩写词语义理解不足的问题。
...
**技术要点**：
- **技术选型**：SQLAlchemy + FastAPI + Vue 3 + Ant Design Vue + JSON + Python csv
- **架构设计**：多术语库集合系统，四层架构（前端层 → API层 → 服务层 → 数据层）
- **核心接口**：GlossaryCollectionService（术语库管理）、GlossaryTermService（术语CRUD）、GlossaryService（术语匹配和扩展）
- **数据表设计**：glossary_collections（术语库集合表）、glossary_terms（术语表）
- **搜索集成**：修改ChunkSearchService，集成术语扩展功能
- **数据格式**：JSON存储同义词数组，CSV导入/导出支持

**性能指标**：
- 术语匹配时间 < 50ms
- 批量查询吞吐量 > 1000 terms/s
- CSV导入速度 > 100 terms/min
- 搜索延迟增加 < 100ms

**开发周期**：2-3周
**预计上线时间**：2026年4月底

📖 **详细技术方案**：[专业术语库系统技术方案](特性开发/search-optimization-glossary/search-optimization-glossary-03-技术方案.md)

---

### 9.8 UI 视觉系统升级 v2.0 🚧 开发中

**功能定位**：为小遥搜索 v2.0 里程碑版本进行全面的 UI 视觉系统升级，采用 Notion 温暖明亮设计风格，建立完整的设计系统规范，提升产品品质和用户体验。

**核心价值**：
- **品牌价值提升**：通过统一且独特的设计语言，建立更强的品牌辨识度
- **用户体验改善**：更清晰的视觉层次和更舒适的界面设计，提升用户满意度和留存率
- **竞争力增强**：在同类型桌面搜索产品中，通过精致的 UI 设计建立竞争优势
- **里程碑意义**：v2.0 版本作为重要里程碑，UI 升级是版本升级的重要组成部分

**技术要点**：
- **技术选型**：CSS Variables + Design Tokens + Google Fonts + Iconify
- **架构设计**：四层架构（表现层 → 组件层 → 设计系统层 → 资源层）
- **设计系统**：完整的 CSS 变量系统（色彩、字体、间距、圆角、阴影）
- **字体系统**：Nunito（正文）+ Work Sans（标题）字体组合，Google Fonts CDN 加载
- **图标系统**：Lucide Icons + Iconify 集成，支持 1000+ 图标按需加载
- **动画系统**：CSS3 Animations + Transitions，流畅的交互动画
- **响应式**：断点系统 + 移动端适配，确保 800px-1920px 范围内良好显示
...
**核心组件**：
- **XiHeader**：顶部导航栏，56px 高度，优雅的悬停效果
- **XiSearchBox**：搜索框组件，蓝色光晕焦点效果
- **XiResultCard**：结果卡片，12px 圆角，多层阴影系统
- **XiIcon**：图标组件，Iconify 集成，支持尺寸和颜色自定义
...
**兼容性**：
- **浏览器**：Chrome 88+、Edge 88+、Firefox 85+（Electron 内置）
- **优雅降级**：字体加载失败时回退到系统字体
- **移动端适配**：768px 断点，完整响应式支持

**技术约束**：
- **纯前端升级**：不涉及后端 API 变更
- **向后兼容**：不破坏现有功能逻辑
- **性能优先**：渐进增强，字体加载 < 500ms，首屏渲染 < 2s

**开发周期**：2-3周
**预计上线时间**：2026年Q2

📖 **详细技术方案**：[UI 视觉系统升级 v2.0 技术方案](特性开发/ui-update/ui-update-03-技术方案.md)
