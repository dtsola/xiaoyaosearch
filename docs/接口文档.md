    ...
    "models": [
      {
        ...
        "status": "loaded",
        "memory_usage": "2.1GB",
        "load_time": "2025-01-01T10:00:00Z"
      },
      {
        "id": 4,
        "model_type": "speech",
...
```json
{
  ...
  "data": {
    "available_models": [
      {
        "model_type": "embedding",
        "models": ["bge-m3", "text-embedding-ada-002"]
      },
      {
        "model_type": "speech",
        "models": ["faster-whisper", "whisper-1", "aliyun-asr"]
      },
      {
        "model_type": "vision",
        "models": ["cn-clip", "gpt-4-vision"]
      }
    ]
  }
}
...
### 8.3 加载指定模型
```http
POST /api/ai-models/load/{model_id}
```
...
### 8.4 卸载指定模型
```http
POST /api/ai-models/unload/{model_id}
```
...
- **AI模型API** (9/9): 100% 完成
- **系统API** (7/7): 100% 完成
  - ✅ GET /api/system/logs/download - 日志文件下载 *(2025-11-25实现)*
- **配置API** (6/6): 100% 完成
