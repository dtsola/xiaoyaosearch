...
# 步骤3: 返回结果（包含扩展信息，但未用于搜索）
{
    "results": [...],
    "glossary_expansion": {
        "matched_terms": [...],
        "expanded_queries": [...]  # 扩展词已返回，但未用于搜索
    }
}
```

**场景1+：文本输入 + 术语扩展（规划中：真正的并发搜索）**
```python
# 用户输入: "PRD"
# 输入类型: TEXT
...
class ChunkSearchService:
    ...
    async def search(self, query: str, search_type: SearchType,
        ...
        if self.enable_glossary_expansion and input_type in [InputType.TEXT, InputType.VOICE]:
            try:
                ...
                if glossary_result.matched_terms:
                    ...
                    # ⚠️ 当前实现：使用原查询搜索，返回扩展信息供前端展示
                    # 📋 规划中（v2.4）：实现并发搜索多个扩展词
        ...
        # 格式化响应，附加术语扩展信息
        return self._format_compatible_response(results, query, response_time, search_type, glossary_info)
```
...
class ChunkSearchService:
    ...
    async def search_with_expansion(self, query: str, search_type: SearchType,
                                  input_type: InputType = InputType.TEXT,
                                  limit: int = 20, threshold: float = 0.7,
                                  filters: List[str] = None):
        """执行术语扩展并发搜索"""
...
### 更新历史记录
- **v2.4** (2026-04-12) - 术语扩展并发搜索规划版
  - 📋 **架构决策**: AD-20260412-01 - 术语扩展并发搜索方案
  - ⏸️ **并发搜索**: 规划中，使用asyncio.gather()并发搜索多个扩展词
  - ⏸️ **结果合并**: 规划中，按file_id去重并取最高分
  - ⏸️ **性能优化**: 预期延迟增加50-200ms，召回率提升60%
...
> **最后更新时间**: 2026年04月12日
> **文档版本**: v2.4 - 术语扩展并发搜索规划版
> **核心特性**: 多模态AI智能搜索系统 + 专业术语库扩展（规划并发搜索） + 分块搜索精度提升80% + 轮询架构优化
> **技术栈**: BGE-M3 + FasterWhisper + Chinese-CLIP + Faiss + Whoosh + 术语库系统
> **重要特性**:
...
> - ⏸️ **并发搜索（规划中）**: 使用asyncio并发搜索多个扩展词，召回率预计+60%
> - 📋 **架构决策**: AD-20260412-01 - 术语扩展并发搜索方案
> **性能指标**: 搜索精度95%、平均响应80-150ms、术语扩展延迟+5ms、召回率提升40%（当前）/60%（规划中）
>
> **术语扩展特性（当前实现 v2.3）**:
> - 支持50+术语库，UI虚拟滚动和分页优化
> - 搜索时自动匹配术语，返回同义词列表
> - 可选择特定术语库进行扩展（如只使用医疗+IT术语库）
> - 扩展信息随搜索结果返回，前端可展示匹配的术语
> - ⚠️ **部分实现**: 当前仅返回扩展信息，未使用扩展词进行搜索
>
> **术语扩展特性（规划中 v2.4）**:
> - 并发搜索多个扩展词（默认5个：原词+4个扩展词）
> - 智能结果合并：按file_id去重，取最高相关性分数
> - 扩展词数量可配置（环境变量：GLOSSARY_MAX_EXPANSION_TERMS）
> - 预期延迟增加：50-200ms
> - 预期召回率提升：60%（相比不使用扩展）
