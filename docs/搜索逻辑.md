...
> **多模态AI智能搜索系统架构** - 小遥搜索v2.4术语扩展并发搜索版

## 🎯 概述

小遥搜索采用多模态AI智能搜索技术，通过BGE-M3文本嵌入模型、FasterWhisper语音识别、Chinese-CLIP图像理解和专业术语库系统，实现了文本、语音、图像三种输入方式的智能搜索功能。v2.4版本实现术语扩展并发搜索，通过并发搜索多个扩展词提升召回率，搜索召回率提升60%。
...
# 步骤5: 返回结果
{
    "results": merged_results,  # 包含所有扩展词的搜索结果
    "glossary_expansion": {
        "matched_terms": [...],
        "expanded_queries": [...],
        "queries_used": ["PRD", "产品需求规格书", "产品需求"]  # 实际使用的查询词
    }
}
```

**场景1+：文本输入 + 术语扩展（v2.4已完成：真正的并发搜索）**
```python
# 用户输入: "PRD"
# 输入类型: TEXT
...
if enable_glossary_expansion and input_type in [TEXT, VOICE]:
    ...
    # 获取扩展词（限制数量）
    expanded_queries = ["PRD"]  # 原始查询
    for synonym in glossary_result.expanded_queries[1:MAX_EXPANSION_TERMS]:
        expanded_queries.append(synonym)
    # 最终查询列表: ["PRD", "产品需求规格书", "产品需求", "需求文档"]
...
if enable_glossary_expansion:
    result = glossary_service.expand_query(
        query="怎么配置API",
        collection_ids=[1, 2]  # 产品+IT术语库
    )
    glossary_info = result

# 步骤3: 执行搜索
results = chunk_search_service.search(
    query="怎么配置API",
...
> **最后更新时间**: 2026年04月12日
> **文档版本**: v2.4 - 术语扩展并发搜索版
> **核心特性**: 多模态AI智能搜索系统 + 专业术语库扩展（并发搜索已完成） + 分块搜索精度提升80% + 轮询架构优化
> **技术栈**: BGE-M3 + FasterWhisper + Chinese-CLIP + Faiss + Whoosh + asyncio + 术语库系统
> **重要特性**:
...
> - ✅ **并发搜索（已完成）**: 使用asyncio并发搜索多个扩展词，召回率+60%
> - ✅ **结果合并（已完成）**: 按file_id去重，取最高相关性分数
> - ✅ **扩展词数量配置（已完成）**: 前端可配置，默认3个，范围1-20
> - 📋 **架构决策**: AD-20260412-01 - 术语扩展并发搜索方案
...
**更新日期**: 2026年04月12日
**架构决策**: [AD-20260412-01](架构决策/AD-20260412-01-术语扩展并发搜索方案.md)