...
erDiagram
    ...
    %% glossary_collections表索引 (v4.1新增):
    %% PRIMARY KEY (id)
    %% UNIQUE KEY (name)
    %% INDEX (is_enabled)
    %% INDEX (is_system)

    %% glossary_terms表索引 (v4.1新增):
    %% PRIMARY KEY (id)
...
**config_json 字段结构说明 (v4.0新增)**

`config_json` 字段通过 JSON 格式灵活存储不同 provider 的差异化配置：
...
**provider='cloud' 的 JSON 结构（v4.0新增）**
...
**本地/云端互斥约束**：对于 `model_type='embedding'` 的配置，同一时间只能有一个 `is_active=true` 的记录。

**API 密钥加密约束**：`api_key` 字段在存储到 `config_json` 前必须使用 AES 加密，加密密钥从环境变量 `API_ENCRYPTION_KEY` 读取。

**配置缓存 (v4.0新增)**

建议将活跃的嵌入模型配置缓存到内存（TTL 5分钟），减少数据库查询频率。
...
|------------|---------------|--------------|------|
| default_glossary_collections | "all" 或 "[1,2,3]" | json | 默认使用的术语库ID列表，"all"表示全部 (v4.1新增) |
| video_frame_search_enabled | "false" | boolean | 是否启用视频画面搜索 |
| video_frame_interval | "10" | integer | 关键帧提取间隔（秒） |
| video_frame_max_duration | "600" | integer | 最大处理时长（秒） |
| video_frame_resolution | "224" | integer | 帧分辨率（像素） |
...
#### 2.2.9 glossary_collections 术语库集合表 (v4.1新增)
存储术语库的基本信息和配置。
...
#### 2.2.10 glossary_terms 术语表 (v4.1新增)
...
1. **自动更新术语数量触发器**
   - 插入术语时自动增加术语库的term_count
   - 删除术语时自动减少术语库的term_count
   - 更新术语启用状态时自动调整term_count
...
document_schema = Schema(
    ...
    # 数据源信息（插件系统）
    source_type=ID(stored=True),                             # 数据源类型（filesystem/yuque/feishu/notion）
    source_url=ID(stored=True),                              # 数据源原始URL
...
| glossary_collections | GlossaryCollectionModel | glossary_collections | 🚧 设计中 | 2026-04-11 |
| glossary_terms | GlossaryTermModel | glossary_terms | 🚧 设计中 | 2026-04-11 |
...
#### ai_models表字段扩展历史 (v4.0新增)

**v4.0新增云端嵌入模型配置支持 (2026-03-26)**:
- ✅ `provider='cloud'` 的 config_json 结构：支持 API 密钥、端点、模型名称等配置
- ✅ API 密钥加密存储：使用 AES 加密，密钥从环境变量 `API_ENCRYPTION_KEY` 读取
- ✅ 本地/云端互斥约束：同一时间只能有一个活跃的 embedding 配置
- ✅ 支持多云端服务商：OpenAI、DeepSeek、阿里云通义千问等
- ✅ 无需数据库迁移：完全复用现有 ai_models 表结构
...
- v4.0 (云端嵌入模型): 设计完成，待实现 *(2026-03-26)*
- v4.1 (专业术语库系统): 设计完成，待实现 *(2026-04-11)*
...
| API接口 | 数据库操作 | 对应表 | 说明 |
|---------|-----------|--------|------|
| PUT /api/config/ai-model | INSERT/UPDATE ai_models SET provider='cloud', config_json='...' | ai_models | 配置云端嵌入模型（自动加密api_key） |
| GET /api/config/ai-models/providers | SELECT DISTINCT provider FROM ai_models WHERE model_type='embedding' | ai_models | 查询可用的嵌入模型提供商 |
| PUT /api/config/ai-model/{model_id}/test-connection | 测试云端API连接（仅验证，不写数据库） | - | 测试云端模型连接状态 |
...
19. **✅ v4.0新增: ai_models表扩展config_json字段结构** - 支持云端嵌入模型配置，无需表结构变更 *(2026-03-26)*
20. **✅ v4.0新增: 云端嵌入模型provider='cloud'配置** - 支持OpenAI/DeepSeek/阿里云等API *(2026-03-26)*
21. **✅ v4.0新增: API密钥加密存储机制** - config_json中api_key使用AES加密 *(2026-03-26)*
22. **✅ v4.0新增: 本地/云端嵌入模型互斥约束** - 确保同一时间只有一个活跃配置 *(2026-03-26)*
23. **🚧 v4.1新增: 专业术语库系统** - glossary_collections和glossary_terms表设计完成 *(2026-04-11)*
24. **🚧 v4.1新增: 术语库触发器设计** - 自动维护term_count和时间戳 *(2026-04-11)*
25. **🚧 v4.1新增: app_settings扩展配置** - default_glossary_collections配置项 *(2026-04-11)*
26. **🚧 v4.1新增: 术语同义词支持** - JSON数组存储多个同义词 *(2026-04-11)*
27. **🚧 v4.1新增: 预置术语库设计** - 系统4个预置术语库（医疗/法律/产品/IT） *(2026-04-11)*
...
- ☁️ **v4.0设计里程碑**: 云端嵌入模型数据库设计100%完成 - 无需迁移，复用现有ai_models表 *(2026-03-26)*
- 📚 **v4.1设计里程碑**: 专业术语库系统数据库设计100%完成 - 支持多领域术语管理和查询扩展 *(2026-04-11)*